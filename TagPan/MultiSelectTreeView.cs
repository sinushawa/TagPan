...
namespace TagPan
{
    public class MSTreeview : MaxEditableTreeView
    {
        public List<TreeNode> HilightedNodes = new List<TreeNode>();

        #region Selected Node(s) Properties

        private List<System.Windows.Forms.TreeNode> m_SelectedNodes = new List<System.Windows.Forms.TreeNode>();
        public List<System.Windows.Forms.TreeNode> SelectedNodes
        {
            get
            {
        ...
        public delegate void DClickHandler(object sender, EventArgs e);
            ...
            try
            {
                ...
                if (node != null)
                {
                    ...
                    if (e.Location.X > leftBound && e.Location.X < rightBound)
                    {
                        ...
                        if (ModifierKeys == Keys.None && (m_SelectedNodes.Contains(node)) && e.Button== System.Windows.Forms.MouseButtons.Left)
                        {
                            // Potential Drag Operation
                            // Let Mouse Up do select
                            this.DoDragDrop(node, DragDropEffects.Copy);
                        }
                        else if (!((m_SelectedNodes.Contains(node) && e.Button== System.Windows.Forms.MouseButtons.Right)))
                        {
                            SelectNode(node);
                        }
                    }
        ...
        protected override void OnDragDrop(DragEventArgs e)
        {
            ...
            TreeNode _targetNode = this.GetNodeAt(pt);
            TreeNode _dragedNode = (TreeNode)e.Data.GetData(typeof(TreeNode));
            if (_targetNode != _dragedNode)
            {
                ((TreeNode)e.Data.GetData(typeof(TreeNode))).Parent.Nodes.Remove(((TreeNode)e.Data.GetData(typeof(TreeNode))));
                // Add a child node.
                _targetNode.Nodes.Add((TreeNode)e.Data.GetData(typeof(TreeNode)));
        ...
        public void HighLight(TreeNode _node, bool Hilit)
        {
            if (Hilit)
            {
                if (m_SelectedNodes.Contains(_node))
                {
                    _node.BackColor = Color.FromArgb(87, 128, 126);
                    _node.ForeColor = SystemColors.HighlightText;
                }
                else
                {
                    _node.BackColor = Color.FromArgb(230,255,192);
                    _node.ForeColor = SystemColors.ControlText;
                }
                
                HilightedNodes.Add(_node);
            }
            else
            {
